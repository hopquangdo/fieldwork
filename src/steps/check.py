...
dot_range) vào ``ctx.data["issues"]`` và report ``"vấn đề đầu vào"``. Không sửa gì,
...
def check(ctx) -> None:
    ...
    issues = baseline_issues(
        ctx.data["assign"], ctx.data.get("photos", []), ctx.data.get("hm_dirs", {}), prof,
    )
    ...
    if not issues:
        return
    ctx.report.sections["vấn đề đầu vào"] = [f"{i.hm} · {i.kind} · {i.detail}" for i in issues]
